#!/usr/bin/env bash
# tanlumen installer — downloads latest (or pinned) release from GitHub and installs to ~/.local/bin
#
# usage:
#   curl -fsSL https://raw.githubusercontent.com/tanguc/tanlumen/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/tanguc/tanlumen/main/install.sh | bash -s -- v0.1.1
#   TANLUMEN_PREFIX=/usr/local/bin curl -fsSL .../install.sh | bash

set -euo pipefail

REPO="tanguc/tanlumen"
BIN="tanlumen"
VERSION="${1:-latest}"
PREFIX="${TANLUMEN_PREFIX:-$HOME/.local/bin}"

color() { printf "\033[%sm%s\033[0m" "$1" "$2"; }
info()  { echo "$(color "1;34" "==>") $*"; }
ok()    { echo "$(color "1;32" "✓") $*"; }
warn()  { echo "$(color "1;33" "!") $*" >&2; }
die()   { echo "$(color "1;31" "✗") $*" >&2; exit 1; }

[[ "$(uname -s)" == "Darwin" ]] || die "tanlumen is macOS-only"
command -v curl >/dev/null || die "curl is required"

info "resolving release ($VERSION) of $REPO"
if [[ "$VERSION" == "latest" ]]; then
  # follow redirect to /releases/tag/<latest>
  TAG=$(curl -fsSLI -o /dev/null -w "%{url_effective}" "https://github.com/$REPO/releases/latest" \
        | sed -E 's|.*/tag/||')
  [[ -n "$TAG" && "$TAG" != "latest" ]] || die "no releases found for $REPO"
else
  TAG="$VERSION"
fi
ok "tag: $TAG"

TMP=$(mktemp -d); trap 'rm -rf "$TMP"' EXIT
BASE="https://github.com/$REPO/releases/download/$TAG"

info "downloading $BIN"
curl -fsSL -o "$TMP/$BIN" "$BASE/$BIN" || die "download failed: $BASE/$BIN"

info "downloading SHA256SUMS"
if curl -fsSL -o "$TMP/SHA256SUMS" "$BASE/SHA256SUMS"; then
  info "verifying checksum"
  ( cd "$TMP" && grep " $BIN\$" SHA256SUMS | shasum -a 256 -c - >/dev/null ) \
    && ok "checksum ok" \
    || die "checksum mismatch"
else
  warn "no SHA256SUMS in release — skipping verification"
fi

mkdir -p "$PREFIX"
install -m 0755 "$TMP/$BIN" "$PREFIX/$BIN"
ok "installed: $PREFIX/$BIN"

if ! echo ":$PATH:" | grep -q ":$PREFIX:"; then
  warn "$PREFIX is not in your PATH — add to your shell rc:"
  echo "    export PATH=\"$PREFIX:\$PATH\""
fi

echo
"$PREFIX/$BIN" version || true
echo
info "try it: $BIN          (interactive picker)"
info "or:    $BIN list      (see all presets)"
